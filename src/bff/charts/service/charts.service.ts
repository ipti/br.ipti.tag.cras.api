...
export class ChartsService {
  ...
  async countFamily(request: Request, attendance_unity_fk?: string) {
    if (
      request.user.attendance_unity_ids.length === 0 &&
    ...
    try {
      ...
      `;
        ...
        const ids = Prisma.join(request.user.attendance_unity_ids);
        const counter = await this.prismaService.$queryRaw`
        SELECT COUNT(id) as count FROM family f
        WHERE f.attendance_unity_fk IN (${ids}) AND f.isActive = true
      `;
    ...
    try {
      ...
      `;
        ...
        FROM (
          ...
          FROM family
          INNER JOIN user_identify ON family.id = user_identify.family_fk
          WHERE family.attendance_unity_fk IN (${ids})
          GROUP BY family.id
          HAVING COUNT(*) = 1
        ) AS subquery;
      `;
  ...
  async attendanceFinishedOrPending(
    ...
    if (attendance_unity_fk !== undefined) {
      ...
      COUNT(*) as total
      FROM attendance a
      WHERE a.attendance_unity_fk IN (${ids})
      AND YEAR(a.date) = ${year}
  ...
  async attendanceByMonth(
    ...
    `;
      ...
      LEFT JOIN attendance a ON MONTH(a.date) = months.month AND YEAR(a.date) = ${year} AND a.attendance_unity_fk IN (${ids})
      GROUP BY months.name
      ORDER BY months.month
    `;
