...
export const UserIdentifyServices = () => {
  ...
  const validUserIdentifyToCreate = async (body: IdendifyAttributs) => {
    ...
    if (validUser) {
      const error: ErrorType = makeErrorMessage("Username already exists", 409);
      throw error;
    }
    ...
    try {
      ...
      const err: ErrorType = makeErrorMessage(`${error}`, 404);
      throw err;
      console.log(error);
    }
    // return result;
  ...
  const getUserIdentifyByUserEmail = async (cpf: string) => {
    const idusuario: any = await identificacao_usuario.findOne({
      where: { cpf },
    });
    return idusuario;
  };
  ...
  const updateUserIdentify = async (id: string, body: IdendifyAttributs) => {
    try {
      const userIdentifyFounded = await getUserIdentifyById(id);
      ...
      const transaction = await sequelize.transaction(async (t) => {
        await endereco.update(
          ...
          { where: { id: userIdentifyFounded.id_endereco }, transaction: t } // Adicione 'transaction: t' aqui
        );

        await situacao_financeira.update(
          {
            profissao: body.profissao,
            renda: body.renda,
            reside_familia: body.reside_familia,
          ...
          {
            where: { id: userIdentifyFounded.id_situacao_financeira },
            transaction: t,
          } // Adicione 'transaction: t' aqui
        );

        await vulnerabilidade.update(
          {
            ocupacao_irregular: body.ocupacao_irregular,
            crianca_sozinha: body.crianca_sozinha,
            idosos_dependentes: body.idosos_dependentes,
          ...
          {
            where: { id: userIdentifyFounded.id_vulnerabilidade },
            transaction: t,
          } // Adicione 'transaction: t' aqui
        );

        await identificacao_usuario.update(
          {
            pasta: body.pasta,
            arquivo: body.arquivo,
            nº: body.nº,
  ...
  return {
    validUserIdentifyToCreate,
    getAllUserIdentify,
    updateUserIdentify,
    deleteUserIdentify,
    getUserIdentifyById,
    getUserIdentifyByUserEmail,
  };
};
